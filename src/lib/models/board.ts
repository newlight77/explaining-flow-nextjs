...
export class BoardImpl implements Board {
  ...
  items(): WorkItem[][] {
    return this.columns.map(column => column.items());
  }
  
  size(): number {
    return this.columns.map(column => column.size())
  ...
  private initialize(workColumnNames: string[]): void {
    ...
    this.columns = factory.createColumns(workColumnNames);
    events.boardReady.next({ columns: this.columns });
  }
  ...
  private workColumns(): WorkList[] {
    return this.columns.filter((_, index) => index > 0 && index < this.columns.length - 1);
  }
    ...
    if (columnWithWork) {
      ...
      if (availableWorker) {
        availableWorker.startWorkingOn(
          columnWithWork, 
          columnWithWork, 
          this.columns[this.columns.indexOf(columnWithWork) + 1]
        );
      }
    }
  }
